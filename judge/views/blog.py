...
class TheoryList(ListView):
    model = TheoryPost
    paginate_by = 10
    context_object_name = 'theorys'
    ...
    def get_queryset(self):
        return (TheoryPost.objects.filter(visible=True, publish_on__lte=timezone.now()).order_by('-sticky', '-publish_on')
                .prefetch_related('authors__user'))

    def get_context_data(self, **kwargs):
        context = super(TheoryList, self).get_context_data(**kwargs)
        context['title'] = self.title or _('Page %d of Theorys') % context['page_obj'].number
        context['first_page_href'] = reverse('home')
        context['page_prefix'] = reverse('blog_post_list')
        context['comments'] = Comment.most_recent(self.request.user, 10)
        context['new_problems'] = Problem.get_public_problems() \
        ...
        context['post_comment_counts'] = {
            int(page[2:]): count for page, count in
            Comment.objects
                   .filter(page__in=['b:%d' % post.id for post in context['theorys']], hidden=False)
                   .values_list('page').annotate(count=Count('page')).order_by()
        }
...
class TheoryView(TitleMixin, CommentedDetailView):
    model = TheoryPost
    pk_url_kwarg = 'id'
    context_object_name = 'theory'
    template_name = 'theory/content.html'
    ...
    def get_object(self, queryset=None):
        post = super(TheoryView, self).get_object(queryset)
        if not post.can_see(self.request.user):
            raise Http404()
        return post
