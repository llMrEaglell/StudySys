...
# class CourseRankedSubmission(ForceContestMixin, RankedSubmissions):
#     def get_title(self):
#         if self.problem.is_accessible_by(self.request.user):
#             return _('Best solutions for %(problem)s in %(course)s') % {
#                 'problem': self.problem_name, 'course': self.contest.name,
#             }
#         return _('Best solutions for problem %(number)s in %(contest)s') % {
#             'number': self.get_problem_number(self.problem), 'contest': self.contest.name,
#         }
